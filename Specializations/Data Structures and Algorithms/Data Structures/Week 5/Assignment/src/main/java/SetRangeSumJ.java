...
public class SetRangeSumJ {
    ...
    void erase(int x) {
        ...
        else if (result.key != x){
            return;
        }
        //find next N which is smallest element in right subtree
        Vertex next = min(root.right);
        splay(next);
        splay(result);
        //now delete
    ...
    SetRangeSumJ() throws IOException {
